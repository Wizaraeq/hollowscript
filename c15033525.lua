...
function c15033525.ccfilter(c)
	return bit.band(c:GetOriginalType(),TYPE_MONSTER)~=0
end
function c15033525.cfilter(c)
	return c:IsLocation(LOCATION_GRAVE) and c:IsType(TYPE_MONSTER)
end
function c15033525.damcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c15033525.ccfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	local g=Duel.GetFieldGroup(tp,LOCATION_MZONE,0)
	g:RemoveCard(e:GetHandler())
	if chk==0 then return g:GetCount()>0 and not g:IsExists(c15033525.ncostfilter,1,nil) end
	Duel.SendtoGrave(g,REASON_COST)
	local ct=g:FilterCount(c15033525.cfilter,nil,LOCATION_GRAVE)
	e:SetLabel(ct)
end
